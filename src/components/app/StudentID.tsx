  ...
  const {loading, submitFunc} = useSubmit(async () => {
    ...
    if (res.status) {
      ...
      switch (res.report) {
        case "invalid_stdID":
          addToast({
            color: "red", icon: "cross", text: "กรุณาลองกรอกใหม่อีกครั้ง รหัสประจำตัวสอบจะต้องมีความยาว 7 หลักและเป็นตัวเลขทั้งหมด",
            theme: "modern", title: "รหัสประจำตัวสอบไม่ถูกต้อง"
          })
          setWarning(true)
          break
        case "missing_stdID":
